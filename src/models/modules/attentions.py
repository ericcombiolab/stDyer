...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        # print("attn.shape, value.shape", attn.shape, value.shape)
        # for multihead scaled dot product attention
        # attn: (BN, 1, K + 1) value: (BN, K + 1, D)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int = 512, num_heads: int = 8):
        ...
        # d_output = np.ceil(d_model / num_heads).item() * num_heads
        d_output = np.ceil(d_model / num_heads) * num_heads
        # assert d_output % num_heads == 0, "d_output % num_heads should be zero."
        self.d_head = int(d_output / num_heads)
        self.num_heads = num_heads
